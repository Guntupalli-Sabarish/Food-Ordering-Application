...
FROM restaurants WHERE name = 'Concu Desserts'
AND NOT EXISTS (SELECT 1 FROM menu_items WHERE item_name = 'Cold Brew Coffee' AND restaurant_id = (SELECT restaurant_id FROM restaurants WHERE name = 'Cold Brew Coffee'));
